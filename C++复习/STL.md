...
# allocator
==设计理念：==
	**allocator是STL中负责内存管理的组建，核心作用是把容器的数据结构逻辑和底层内存操作解耦。容器只关心数据结构（怎么组织元素），allocator只关心内存（从哪里拿内存，怎么还回去）。两者通过模板参数绑定，互不侵入。想换内存策略，只需要换allocator类型，容器代码一行不改。**
	![[Pasted image 20260916190637.png]]

平时写std::vector< int >时省略的第二个参数就是他
## 为什么需要Allocator？
如果容器直接调用new/delete，就写死了内存来源。有了Allocator，容器只需要说“我需要n个T类型的空间”，至于这空间是从堆上malloc来的，从内存池里切的，还是从共享内存里映射的，容器不需要管。
## 主要用到的场景
1.共享内存：让多个进程共享同一个容器数据
2.内存对齐：处理SIMD等需要特殊对齐的场景
3.调试追踪：记录每次分配，排查内存泄漏

**有四个职责（接口）：把==内存的生命周期==和==对象的生命周期==分开。容器可以先分配一大块内存，后面需要时再逐个构造对象，不用每次都new/delete。**
![[Pasted image 20260916191220.png]]
## 1.分配原始内存allocate；
